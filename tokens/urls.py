...
from tokens.views import ActivePromoCodesListApiView, PromoCodesList, EventCreateApiView, PromoCodesListApiView, \
    PromoCodeUpdateApiView, PromoCodeRadiusUpdateApiView, PromoCodeRetrieveApiView
...
urlpatterns = [
    ...
    url(
        r'^api/(?P<origin_lat>.*)/(?P<origin_lon>.*)/(?P<destination_lat>.*)/(?P<destination_lon>.*)/(?P<code>.*)/valid$',
        PromoCodeRetrieveApiView.as_view(), name='valid_code'),
